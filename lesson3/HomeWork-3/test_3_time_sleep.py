...
def test_2_implicit_waits(driver):
    ...
    time.sleep(10)
    visible_button_start_test = driver.find_element(By.XPATH, '// button[@id ="startTest"]')
    ...
    visible_loader = driver.find_element(By.XPATH, "//div[@id ='loader']")
    assert visible_loader
    time.sleep(5)

    """11. Проверка сообщения: Убедиться, что после завершения загрузки появилось сообщение "Вы успешно зарегистрированы"."""
    success_message = driver.find_element(By.XPATH, "//p[@id ='successMessage']")
